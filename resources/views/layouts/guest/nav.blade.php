<div>
    {{-- ------------------------------------------------------------------------------------------------------
    | <meta_config>
        | @path : resources/views/livewire/pages/portfolio/navigation.blade.php
        | @usage : Portfolio Navigation — Desktop Section Navigation, Mobile Navigation & Appearance Control
        | @type : Navigation Component
        |
        | @expected_data : [portfolio sections]
        | @expected_events : [section visibility]
        | @techstack : Laravel, Livewire, Volt, Alpine, Flux UI, Tailwind CSS
        | @convention : Yogawilanda Documentation v1.1.6
        ------------------------------------------------------------------------------------------------------ --}}
        ...
        $sections = [
        'intro' => [
        'label' => __('portfolio.nav.about'),
        'mobile' => 'Intro',
        ],
        'stack' => [
        'label' => __('portfolio.nav.capabilities'),
        'mobile' => 'Stacks',
        ],
        'projects' => [
        'label' => __('portfolio.nav.work'),
        'mobile' => 'Projects',
        ],
        'contact' => [
        'label' => __('portfolio.nav.contact'),
        'mobile' => 'Contact',
        ],
        ];
        @endphp
        ...
        <div class="fixed right-6 top-6 z-50 flex items-center gap-2">
            ...
            <flux:dropdown x-data align="end">
                <flux:button variant="subtle" square class="group size-9" aria-label="Preferred color scheme">
        ...
        <nav class="fixed right-6 top-1/2 z-50 hidden -translate-y-1/2 lg:block" aria-label="Section navigation">
            <div class="flex flex-col items-end gap-3">

                @foreach ($sections as $hash => $section)

                <a href="{{ $isHome ? " #{$hash}" : route('home') . "#{$hash}" }}" data-section="{{ $hash }}"
                    class="nav-link group flex items-center gap-3 py-1 text-sm font-medium text-zinc-400 transition-colors dark:text-zinc-600">
                    <span class="nav-label transition-colors duration-300">
                        {{ $section['label'] }}
                    </span>

                    <span
                        class="nav-indicator h-px w-5 bg-zinc-300 transition-all duration-300 dark:bg-zinc-700"></span>
                </a>
        ...
        {{-- Mobile Navigation --}}
        <nav class="fixed inset-x-0 bottom-5 z-50 flex justify-center px-4 lg:hidden"
        ...
        <script>
            ...
            const setActive = (id) => {
                links.forEach((link) => {
                    ...
                    link.classList.toggle('text-zinc-950', isActive);
                    link.classList.toggle('dark:text-white', isActive);

                    if (indicator) {
                        indicator.classList.toggle('w-8', isActive);
                        indicator.classList.toggle('bg-zinc-950', isActive);
